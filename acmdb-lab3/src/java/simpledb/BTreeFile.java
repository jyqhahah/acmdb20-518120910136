...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (pid.pgcateg() == BTreePageId.LEAF) {
			Page leaf = getPage(tid, dirtypages, pid, perm);
			assert leaf instanceof BTreeLeafPage;
			return (BTreeLeafPage) leaf;
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		Page p = getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		BTreeLeafPage rpage = (BTreeLeafPage)p;
		Iterator<Tuple> iterator = page.reverseIterator();
		int cnt = 0, halfTuple = page.getNumTuples() / 2; Tuple t = iterator.next();
		while(cnt < halfTuple){
			page.deleteTuple(t);
			rpage.insertTuple(t);
			t = iterator.next();
		...
		Field midKey = t.getField(keyField());
		BTreePageId rSiblingId = page.getRightSiblingId();
		rpage.setRightSiblingId(rSiblingId);
		if(rSiblingId != null)
			((BTreeLeafPage) getPage(tid, dirtypages, rSiblingId, Permissions.READ_WRITE)).setLeftSiblingId(rpage.getId());
		page.setRightSiblingId(rpage.getId());
		rpage.setLeftSiblingId(page.getId());
		BTreeEntry entry = new BTreeEntry(midKey, page.getId(), rpage.getId());
		BTreeInternalPage internalPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), entry.getKey());
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		Page p = getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		BTreeInternalPage rpage = (BTreeInternalPage)p;
		Iterator<BTreeEntry> iterator = page.reverseIterator();
		int cnt = 0, halfEntry = page.getNumEntries() / 2; BTreeEntry entry = iterator.next();
		while (cnt < halfEntry){
		...
		page.deleteKeyAndRightChild(entry);
		BTreeEntry nwEntry = new BTreeEntry(entry.getKey(), page.getId(), rpage.getId());
		BTreeInternalPage internalPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), nwEntry.getKey());
		internalPage.insertEntry(nwEntry);
		updateParentPointers(tid, dirtypages, internalPage);
		if(field.compare(Op.LESS_THAN_OR_EQ, nwEntry.getKey()))
			return page;
		return rpage;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numTuples = page.getNumTuples(), halfTuples = (numTuples + sibling.getNumTuples()) / 2;
		if(isRightSibling){
			Iterator<Tuple> iterator = sibling.iterator(); Tuple t = iterator.next();
			while(numTuples < halfTuples){
		...
		Iterator<Tuple> iterator = sibling.reverseIterator(); Tuple t = iterator.next();
		while(numTuples < halfTuples){
			sibling.deleteTuple(t);
			page.insertTuple(t);
			t = iterator.next();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numEntries = page.getNumEntries(), halfEntries = (numEntries + leftSibling.getNumEntries()) / 2;
		Iterator<BTreeEntry> iterator = leftSibling.reverseIterator(); BTreeEntry entry = iterator.next();
		Field key = parentEntry.getKey(); BTreePageId rightChild = page.iterator().next().getLeftChild();
		while(numEntries < halfEntries){
			leftSibling.deleteKeyAndRightChild(entry);
			BTreeEntry nwEntry = new BTreeEntry(key, entry.getRightChild(), rightChild);
			page.insertEntry(nwEntry);
			key = entry.getKey();
			rightChild = entry.getRightChild();
			entry = iterator.next();
			++numEntries;
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numEntries = page.getNumEntries(), halfEntries = (numEntries + rightSibling.getNumEntries()) / 2;
		Iterator<BTreeEntry> iterator = rightSibling.iterator(); BTreeEntry entry = iterator.next();
		Field key = parentEntry.getKey(); BTreePageId leftChild = page.reverseIterator().next().getRightChild();
		while(numEntries < halfEntries){
			rightSibling.deleteKeyAndLeftChild(entry);
			BTreeEntry nwEntry = new BTreeEntry(key, leftChild, entry.getLeftChild());
			page.insertEntry(nwEntry);
			key = entry.getKey();
			leftChild = entry.getLeftChild();
			entry = iterator.next();
			++numEntries;
		}
		parentEntry.setKey(key);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<Tuple> iterator = rightPage.iterator(); Tuple t;
		...
		if(rightId != null)
			((BTreeLeafPage)getPage(tid, dirtypages, rightId, Permissions.READ_WRITE)).setLeftSiblingId(leftPage.getId());
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		BTreeEntry nwEntry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(nwEntry);
		Iterator<BTreeEntry> iterator = rightPage.iterator(); BTreeEntry entry;
		while(iterator.hasNext()){
			entry = iterator.next();
