...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp == Predicate.Op.EQUALS){
            ...
            int y = estimateTableJoinCardinality(Predicate.Op.EQUALS, table1Alias, table2Alias, field1PureName, field2PureName, card1, card2, false, false, stats, tableAliasToId);
            return Math.max(x, y);
        }
    }
        ...
        for(int i=1; i<=joins.size(); ++i){
            ...
            for(Set<LogicalJoinNode> subSet : subSets){
                ...
                for(LogicalJoinNode ljNode : subSet){
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, ljNode, subSet, bestPlan.cost, planCache);
                    if(cc != null) bestPlan = cc;
                }
